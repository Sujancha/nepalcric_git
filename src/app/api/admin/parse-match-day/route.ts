...
export async function GET() {
  ...
  const filePath = `content/pages/match-day.json`;
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch or parse match-day.json', detail: message }, { status: 500 });
  }
}

// POST { data: { targetDate, fixtures: [...] }, sha }
export async function POST(request: NextRequest) {
  ...
  try {
    const putRes = await fetch(putUrl, {
      ...
      headers: githubHeaders(token),
      body: JSON.stringify({
        message: `admin: update match-day config`,
